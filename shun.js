...
await hardi.sendMessage(m.chat,{ video: {url:vido.videoUrl}, caption: autod },{quoted: fcall })
}
...
let msg = generateWAMessageFromContent(m.chat, {
 ...
 subtitle: "LynzzDev",
 hasMediaAttachment: true,...(await prepareWAMessageMedia({ image: { url: "https://telegra.ph/file/bc9d083e9da67d22d5606.png" } }, { upload: hardi.waUploadToServer }))
 }),
...
wek = `┌└─────────────────≽
││ 「 Info Mancing 」
││👑 ᴄʀᴇᴀᴛᴏʀ : *@${owned.split("@")[0]}*
││🤖 ᴍᴏᴅᴇ ʙᴏᴛ : *${hardi.public ? `ᴘᴜʙʟɪᴄ` : `sᴇʟғ`}*
...
"buttonParamsJson": JSON.stringify(listMessage) 
 ...
 {
 "name": "cta_url",
 "buttonParamsJson": "{\"display_text\":\"Creator\",\"url\":\"https://wa.me/6281331645801\",\"merchant_url\":\"https://wa.me/6281331645801\"}"
 },
 ],
 })
 })
...
body: `LynzzDev`,
thumbnail: thumb,
sourceUrl: 'https://chat.whatsapp.com/BZze2IvOVpjDaMUzWdZqZn', 
mediaType: 1,
...
hardi.sendMessage(from, { contextInfo: { externalAdReply: { showAdAttribution: true,
...
await hardi.group (m.chat).then((res) => reply(jsonformat(res))).catch((err) => reply(jsonformat(err)))
}
break
...
item3.URL:https://store.lynz.biz
...
hardi.sendMessage(from, { text : `Hai Kak @${sender.split("@")[0]}, Nih Owner Ku.`, contextInfo:{
...
if (args[0] === 'disable'){
await hardi.groupSettingUpdate(from, 'announcement').then((res) => reply(`Sukses Menutup Group`)).catch((err) => reply(jsonformat(err)))
} else if (args[0] === 'enable'){
await hardi.groupSettingUpdate(from, 'not_announcement').then((res) => reply(`Sukses Membuka Group`)).catch((err) => reply(jsonformat(err)))
} else {
hardi.sendMessage(m.chat, { image: ppnyauser, caption: ` Silahkan Ketik
Group Open
...
wlcm.push(from)
var groupe = await hardi.groupMetadata(from)
var members = groupe['participants']
var mems = []
members.map(async adm => {
mems.push(adm.id.replace('c.us', 's.whatsapp.net'))
})
hardi.sendMessage(from, {text: `Fitur Welcome Di Aktifkan Di Group Ini`, contextInfo: { mentionedJid : mems }}, {quoted:m})
} else if (args[0] === "disable") {
...
case 'backup':
     ...
     await reply("Sabar Mas Lagi Backup!!!");
          ...
          const exec = await execSync(`zip -r Lynz.zip ${ls.join(" ")}`);
...
case "bcimg": {
if (!isCreator) return reply(`Lu Lynzz Kah?`)
...
case "listgc":{
if (!isCreator) return (`Ngapain ? Khusus Lynzz Offcial Aja !!`)
let getGroups = await hardi.groupFetchAllParticipating()
let groups = Object.entries(getGroups).slice(0).map((entry) => entry[1])
let anu = groups.map((v) => v.id)
let hituet = 0
...
case 'smeme': case 'stickermeme': case 'stickmeme': {
    ...
    if (!/webp/.test(mime) && /image/.test(mime)) {
        ...
        reply(`Send/reply image with caption ${prefix + command} Lynzz`)
    }
}
    break
